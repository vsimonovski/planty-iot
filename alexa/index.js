...
function onIntent(intentRequest, session, callback) {
    ...
    if (  intentName == "sportIntent" || intentName == "apiIntent" || intentName == "productsIntent" || intentName == "plantyIntentStart" || intentName == "plantyIntentStop") {
...
function handleUnreckognizedIntent() {
    var speechOutput = "Sorry, I don't understand your request. If you don't know what I am capable for, ask me for help."
    callback(buildSpeechletResponseWithoutCard(speechOutput, "Please tell me what do you need.", false))
}
...
function getJSON(callback, calling, intent) {
    if (calling === "sportIntent") {
        ...
        //var infoResponses = ["Do you need something more?", "Do you need anything else?", "Anything else?", "Can I help you with something else?", "Any other question?"]
        //var response = infoResponses[Math.floor(Math.random() * infoResponses.length)]
        var speechOutput = "Job offer sent successfuly. Can I help you with something else?";
        ...
        var test = new Promise(function(resolve, reject) {
            //authorize(function(err, res) {
                ...
                request({
                    ...
                    url: 'https://api.github.com/search/users?q=location:'+country+'+language:'+lang+'&page=1&access_token=f3cf2928fb4f4646eb883b951f970a1127ab79ba',
                    method: 'get'
                }, function(err, res, body) {
                    resolve(JSON.parse(body));
                });
        ...
        test.then(function(res){
            ...
            //var l = 5;
            temp = "There are "+res.total_count+" "+lang+" developers from "+country+" and I think this "+l+" can fit the best. ";
            // for(var i=0; i<l; i++){
            //     if(i==l-1){
            ...
            Promise.all(promises).then(function(data){
                ...
                if(user.hireable===true && counter<l){
                    ...
                    //    if(i==l-1){
                            ...
                            if(user.name==null){
                                temp+=user.login + " from "+user.location+" with "+user.public_repos+" public repos and "+user.followers+" followers.";
                            } else {
                                temp+=user.name + " from "+user.location+" with "+user.public_repos+" public repos and "+user.followers+" followers.";
                            }
